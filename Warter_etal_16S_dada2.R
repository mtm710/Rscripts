...
# path to each plate (plates are numbered 0,1,2,3)

path<- "~/path to plate 0/"
path<- "~/path to plate 1/"
path<- "~/path to plate 2/"
path<- "~/path to plate 3/" 
...
taxa <- assignTaxonomy(seqtab.nochim, "~/path/rdp_train_set_18.fa.gz")
saveRDS(taxa, "taxa.RDS")
write.csv(cbind(t(seqtab.nochim), taxa), "seqtab_nochim_taxa_id.csv", quote=FALSE)

#assign species locally
taxaspecies <- assignSpecies(seqtab.nochim, "~/path/rdp_species_assignment_18.fa.gz")
